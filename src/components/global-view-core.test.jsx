...
import { fetchBusinessDateData, fetchTimestampData } from "../utils/rest-api.js";
...
it('renders global view content', async () => {
    ...
    const client = new Client({
                   ...
                   onConnect: () => {console.info('Connected task-notification Websocket with URL: ');},
                   onStompError: (error) => console.error('Error occurred in task-notification Stomp with URL: '),
                   onWebSocketError: (error) => console.error('Error occurred in task-notification Websocket with URL: '),
               });
    ...
    fetchBusinessDateData.mockImplementation((a,b,c) => Promise.resolve([[{timestamp:refTimestamp}]]));
    fetchTimestampData.mockImplementation((a,b,c) => Promise.resolve([[{timestamp:refTimestamp}]]));

    await renderComponent(<GlobalViewCore timestampMin={refTimestamp.getTime()}
                                          timestampMax={refTimestamp.getTime() + 24 * 60 * 60 * 1000}
                                          timestampStep={'01:00'} />, root);
