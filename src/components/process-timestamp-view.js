...
const ProcessTimestampView = () => {
    ...
    const connectNotificationsUpdateTask = useCallback(() => {
        ...
        ws.onmessage = function (event) {
            ...
            if ( data !== null && data.timestamp === timestamp.toISOString().substr(0, 19)){
                setTaskData(data)
            }
        };
    ...
    const updateSelectedDateData = useCallback((event) => {
    ...
    const updateSelectedTimeData = useCallback((event) => {
    ...
    return (
        <Grid container direction="column">
            <Grid item>
                <TableHeader taskData={taskData} onSelectedDateChange={updateSelectedDateData} onSelectedTimeChange={updateSelectedTimeData}/>
            </Grid>
            <Grid item>
