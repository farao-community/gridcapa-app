import { areSameDates, getDateString, getInitialTimestampToSet, getTimeString, plusOneDay, toISODate } from "./date-time-utils.js";
...
it('should compare dates with or without different types', async () => {
    let timestamp = Date.UTC(2020,0,1);
    expect(areSameDates({timestamp: timestamp},{timestamp: timestamp})).toBe(true);
    expect(areSameDates({timestamp: '2020-01-01T00:00Z'}, {timestamp: timestamp})).toBe(true);
    expect(areSameDates({timestamp: '2025-01-01T00:00Z'}, {timestamp: timestamp})).toBe(false);
});
